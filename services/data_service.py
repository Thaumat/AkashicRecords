...
class DataService(object):
    ...
    def get_known_online_news_journals(self):
    ...
    &start=now-1d    +   &end=now 
        - the date range of the request (starting 24 hours ago and ending now)
    
    
    &count=100 
        - minimum number of results returned (this case 5)
    
    
    &q.enriched.url.enrichedTitle.relations.relation=|action.verb.text=acquire, object.entities.entity.type=Company|
        - return articles having
            - titles with (acquire (as a verb))
            - *or* (entities with (their type) as company))



    &q.enriched.url.concepts.concept.text=football
        - returns articles with football tagged as a football as a concept anywhere in the full body of text
    

    q.enriched.url.enrichedTitle.keywords.keyword.text=paintings
        - returns articles that have a keyword 'paintings' in their title and url
    ...
    def get_articles_by_new_journal_over_timeframe(self, source_url, topic=None, timeSlice="3, ):
